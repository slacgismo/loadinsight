...
class MixedFeederPipeline():
    ...
    def generate_result_plots(self):
        ...
        residential_mix_plots_dir = f'{base.LOCAL_PATH}/{self.run_dir}/suburban_mix_named'
        self._create_results_storage(residential_mix_plots_dir)

        commercial_mix_plots_dir = f'{base.LOCAL_PATH}/{self.run_dir}/urban_mix_named'
        self._create_results_storage(commercial_mix_plots_dir)

        mixed_mix_plots_dir = f'{base.LOCAL_PATH}/{self.run_dir}/mixed_mix_named'
        self._create_results_storage(mixed_mix_plots_dir)

        rural_mix_plots_dir = f'{base.LOCAL_PATH}/{self.run_dir}/rural_mix_named'
        self._create_results_storage(rural_mix_plots_dir)

        residential_mix_hour_norm_plots_dir = f'{base.LOCAL_PATH}/{self.run_dir}/suburban_mix_hour_norm_named'
        self._create_results_storage(residential_mix_hour_norm_plots_dir)

        commercial_mix_hour_norm_plots_dir = f'{base.LOCAL_PATH}/{self.run_dir}/urban_mix_hour_norm_named'
        self._create_results_storage(commercial_mix_hour_norm_plots_dir)

        mixed_mix_hour_norm_plots_dir = f'{base.LOCAL_PATH}/{self.run_dir}/mixed_mix_hour_norm_named'
        self._create_results_storage(mixed_mix_hour_norm_plots_dir)

        rural_mix_hour_norm_plots_dir = f'{base.LOCAL_PATH}/{self.run_dir}/rural_mix_hour_norm_named'
        self._create_results_storage(rural_mix_hour_norm_plots_dir)
        ...
        for idx, city in enumerate(residential_mix_hour_norm.target.unique()):
            ...
            max_val = 1 if max_total <= 1 else int(max_total) + 1
            for ydx, daytype in enumerate(city_df.daytype.unique()):
                title = f'SUBURBAN-hour_norm-{str(city).split(",")[0]}_{str(city).split(",")[1]}-{str(daytype)}'
                day_df = city_df.loc[city_df.daytype == daytype]
                day_df = day_df.append(day_df.iloc[0])
                day_df = day_df.reset_index()
                plot = day_df[plotting_components].plot(kind='area', title=title, grid=True, xticks=ticks, ylim=(0, 1), linewidth=2, color=['green','yellow','brown','blue','grey','black','red'])
        ...
        for idx, city in enumerate(commercial_mix_hour_norm.target.unique()):
            ...
            max_val = 1 if max_total <= 1 else int(max_total) + 1
            for ydx, daytype in enumerate(city_df.daytype.unique()):
                title = f'URBAN-hour_norm-{str(city).split(",")[0]}_{str(city).split(",")[1]}-{str(daytype)}'
                day_df = city_df.loc[city_df.daytype == daytype]
                day_df = day_df.append(day_df.iloc[0])
                day_df = day_df.reset_index()
                plot = day_df[plotting_components].plot(kind='area', title=title, grid=True, xticks=ticks, ylim=(0, 1), linewidth=2, color=['green','yellow','brown','blue','grey','black','red'])
        ...
        for idx, city in enumerate(mixed_mix_hour_norm.target.unique()):
            ...
            max_val = 1 if max_total <= 1 else int(max_total) + 1
            for ydx, daytype in enumerate(city_df.daytype.unique()):
                title = f'MIXED-hour_norm-{str(city).split(",")[0]}_{str(city).split(",")[1]}-{str(daytype)}'
                day_df = city_df.loc[city_df.daytype == daytype]
                day_df = day_df.append(day_df.iloc[0])
                day_df = day_df.reset_index()
                plot = day_df[plotting_components].plot(kind='area', title=title, grid=True, xticks=ticks, ylim=(0, 1), linewidth=2, color=['green','yellow','brown','blue','grey','black','red'])
        ...
        for idx, city in enumerate(rural_mix_hour_norm.target.unique()):
            ...
            max_val = 1 if max_total <= 1 else int(max_total) + 1
            for ydx, daytype in enumerate(city_df.daytype.unique()):
                title = f'RURAL-hour_norm-{str(city).split(",")[0]}_{str(city).split(",")[1]}-{str(daytype)}'
                day_df = city_df.loc[city_df.daytype == daytype]
                day_df = day_df.append(day_df.iloc[0])
                day_df = day_df.reset_index()
                plot = day_df[plotting_components].plot(kind='area', title=title, grid=True, xticks=ticks, ylim=(0, 1), linewidth=2, color=['green','yellow','brown','blue','grey','black','red'])
